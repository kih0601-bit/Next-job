export const STAGE8_QUALITY_AUDIT_VERSION = '1.4.0-title-fallback-accountability';
...
export function auditStage8Quality(report={}, {inputDiagnostics={}}={}) {
  ...
  const lowConfidenceSingles=[],singleDespiteMultiSignal=[],actionableMultiSplit=[],nonBlockingMultiSplit=[],requirementSignalWithoutEvidence=[],unreadRequiredSources=[],actionableUnreadSources=[],historicalUnreadSources=[],informationalUnreadSources=[],noEvidenceUnits=[],nonVacancyUnits=[],actionableNonVacancyUnits=[],titleFallbackUnits=[],actionableTitleFallbackUnits=[];
  for(const {posting,unit} of units){
    ...
    if(unit?.source==='single' && compact(unit?.name,200)===compact(posting?.posting?.title,200)){ const sample=samplePosting(posting,{unit:unit.name,confidence,evidence:compact(evidenceText,700)}); titleFallbackUnits.push(sample); if(!historicalByExplicitYear(posting) && !preliminaryNotice(posting,evidenceText) && !nonTargetOrClosed(posting,evidenceText)) actionableTitleFallbackUnits.push(sample); }
