export const STAGE9_JOB_TAXONOMY_VERSION = '1.0.0-v120';
...
export const JOB_CATEGORIES = Object.freeze([
  ...
  { id:'it-digital', label:'IT·디지털', description:'SW·AI·데이터·정보보안·정보시스템·전산', patterns:[/ICT|SW\b|소프트웨어|AI\b|인공지능|데이터|정보보안|정보기술|전산|네트워크|시스템\s*(?:개발|운영|관리)|디지털/i] },
  ...
  { id:'project-operation', label:'사업기획·운영', description:'공공사업·프로젝트 기획/운영/수행/관리', patterns:[/사업\s*(?:기획|운영|수행|관리|지원)|프로젝트\s*(?:기획|운영|관리)|사업계획/i] },
  { id:'admin-management', label:'사무·행정', description:'일반행정·사무·경영지원·기획·총무·인사', patterns:[/일반행정|행정|사무|경영지원|경영관리|총무|인사|기획조정|사무보조|비서/i] },
...
export function classifyJobCategory({ vacancyName='', title='', localText='' }={}) {
  const primary = `${vacancyName}\n${title}`;
  const fallback = `${primary}\n${localText}`;
  // First pass intentionally uses the unit/title only so a long shared document does not
  // steal the category from the actual recruitment unit.
  for (const category of JOB_CATEGORIES) {
    if (category.id === 'other') continue;
    const pattern = category.patterns.find(p=>p.test(primary));
    if (pattern) return { id:category.id, label:category.label, confidence:'high', matchedOn:'unit-or-title', matchedPattern:String(pattern) };
  }
  for (const category of JOB_CATEGORIES) {
    if (category.id === 'other') continue;
    const pattern = category.patterns.find(p=>p.test(fallback));
    if (pattern) return { id:category.id, label:category.label, confidence:'medium', matchedOn:'evidence', matchedPattern:String(pattern) };
  }
