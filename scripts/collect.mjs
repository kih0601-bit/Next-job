...
import { fetchDetailPage, mergeDetail, parseUipaList } from './lib/detail-parser.mjs';
...
function parseHtml(html, source) {
  if (source.org === '울산정보산업진흥원') return parseUipaList(html, source);
  const jobs = [];
  const seen = new Set();
...
async function fetchSource(source) {
  ...
  try {
    ...
    let jobs = parseHtml(await r.text(), source);

    if (source.org === '울산정보산업진흥원') {
      const detailed = [];
      for (const job of jobs.slice(0, 10)) {
        const detail = await fetchDetailPage(job.link);
        const merged = mergeDetail(job, detail);
        if (merged.excludedByDetail || isExpired(merged.deadline)) continue;
        detailed.push(merged);
      }
      jobs = detailed;
    }

    return { ok: true, source, jobs };
  } catch (e) {
    return {
      ok: false,
      source,
...
const payload = {
  ...
  note: '울산정보산업진흥원은 목록 상태와 상세 HTML을 확인합니다. 다른 기관은 기존 4.1 수집 규칙을 유지합니다.'
};
