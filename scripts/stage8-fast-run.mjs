...
let snapshot;
try { snapshot=JSON.parse(await fs.readFile(SNAPSHOT,'utf8')); }
catch { throw new Error('STAGE8_SNAPSHOT_NOT_AVAILABLE: run the full Update job postings workflow once to create a Stage 7→8 boundary snapshot.'); }
...
report.snapshot={version:snapshot.version||'',generatedAt:snapshot.generatedAt||'',trust:snapshot.trust||{status:'unknown'},inputCount:snapshot.inputs.length};
